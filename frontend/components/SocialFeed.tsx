...
export const SocialFeed: React.FC<SocialFeedProps> = ({ 
    ...
    try {
      ...
      await Promise.all(
        data.map(async (post: any) => {
          const res = await apiFetch(`http://0.0.0.0:8000/api/v1/posts/${post.postId}/comments?limit=10`);
          if (res.ok) {
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      const res = await apiFetch(`http://0.0.0.0:8000/api/v1/posts/${commentingPostId}/comments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
