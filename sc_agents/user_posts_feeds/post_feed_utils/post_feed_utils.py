...
def get_all_posts_summary(gemini_model, all_posts, post_summaries_batch_for_feeds = 20, topk_links = 3):
    ...
    for category, posts in grouped_categories.items():
        ...
        for batch_content in contents:
            ...
            json_text = re.search(r'\{.*\}', reponse.text, re.DOTALL)
            output_json = convert_str_to_json(json_text.group())
            final_summary = output_json["summary"]
            get_all_links.extend(output_json["references"]["links"])
        output_json.update({
            "type": curr_type, 
            "category": category, 
            "location": location_neighborhood,
        ...
        output_json["references"]["links"] = links
    return summaries